...
from .estimators import fit_estimators
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="rq2", description="RQ2 deterministic/stochastic fusion pipeline"
    )
...
def main() -> None:
    ...
    if arguments.command == "extract":
        ...
        print(json.dumps({"models": sorted(fit_estimators(config))}, indent=2))
    elif arguments.command == "evaluate":
        print(json.dumps(evaluate_estimators(config), indent=2))
        ...
        destination = project_path(config, "RQ2/outputs/smoke_features.parquet")
        print(
            extract_split(
                config,
                "train",
                arguments.images,
                destination,
                shared_cache_namespace="rq2_cli_smoke",
            )
        )
