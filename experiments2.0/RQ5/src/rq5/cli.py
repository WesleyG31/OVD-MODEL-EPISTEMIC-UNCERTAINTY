...
from .fusion import fit_policies
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="rq5", description="RQ5 risk-aware selective-perception pipeline"
    )
...
def main() -> None:
    ...
    if arguments.command == "extract":
        ...
        print(json.dumps({"models": sorted(fit_policies(config))}, indent=2))
    elif arguments.command == "evaluate":
        print(json.dumps(evaluate_policies(config), indent=2))
        ...
        destination = project_path(config, "RQ5/outputs/smoke_features.parquet")
        print(
            extract_split(
                config,
                "train",
